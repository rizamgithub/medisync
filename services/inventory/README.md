...
**blood/organ stock units**: geohash-bucketed region search and the
ETag-guarded *reserve* transition consumed by the match service's Saga.
...
│   ├── repository.py          ← Cosmos access (region query + ETag reserve)
...
- **Authentication.** HTTP auth level is `ANONYMOUS`; real authorization is
  ...
  runbook.
- **Infrastructure.** The Function App, its Storage Account, Managed Identity
  and the `inventory` Cosmos container (partition key `/geohash_prefix`) are
  not in `infra/` yet. Note: the container provisioned in runbook 06 is
  `donors` (partitioned `/bloodType`) — the inventory container is a separate,
  geohash-partitioned container the Terraform step will add.
